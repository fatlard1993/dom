...
const dom = {
	...
	basicTextElem: options => {
		return Object.assign({ type: 'text', autocomplete: 'off', autocapitalize: 'off', autocorrect: 'off' }, options);
	},
	style: (elem, styleObj) => {
		for (let x = 0, keys = Object.keys(styleObj), count = keys.length; x < count; ++x) elem.style[keys[x]] = styleObj[keys[x]];
	},
	data: (elem, dataObj) => {
		for (let x = 0, keys = Object.keys(dataObj), count = keys.length; x < count; ++x) elem.dataset[keys[x]] = dataObj[keys[x]];
	},
	attr: (elem, attrObj) => {
		for (let x = 0, keys = Object.keys(attrObj), count = keys.length; x < count; ++x) elem.setAttribute([keys[x]], attrObj[keys[x]]);
	},
	options: (elem, options, ...args) => {
		options = options && options instanceof Array ? options : [options, ...args];

		dom.appendChildren(
			elem,
			options.map(option => {
				return dom.createElem('option', { textContent: option });
			}),
		);
	},
	...
	appendToLabel: (elem, text, wrapper) => {
	...
	isDescendantOf: (elem, parent) => {
		...
		return !isTheParent && elem.parentElement.parentElement ? dom.isDescendantOf(elem.parentElement.parentElement, parent) : isTheParent;
	},
	getScrollbarSize: () => {
	...
	fadeOut: (startDate, rate = 1e10, currentFakeDate) => {
		startDate = new Date(startDate).getTime();
		const currentDate = typeof currentFakeDate !== 'undefined' ? new Date(currentFakeDate).getTime() : new Date().getTime();
	...
	animation: {
		...
		schedule: () => {
			...
			(window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.msRequestAnimationFrame || (cb => setTimeout(cb, 16)))(dom.animation.runner);
		},
	},
	maintenance: {
